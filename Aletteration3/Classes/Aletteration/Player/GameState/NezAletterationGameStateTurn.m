...
-(id)initWithCoder:(NSCoder*)decoder {
	if ((self = [super init])) {
		_lineIndex = [decoder decodeInt32ForKey:@"_lineIndex"];
		_temporaryLineIndex = [decoder decodeInt32ForKey:@"_temporaryLineIndex"];
...
-(void)encodeWithCoder:(NSCoder*)encoder {
	[encoder encodeInt32:_lineIndex forKey:@"_lineIndex"];
	[encoder encodeInt32:_temporaryLineIndex forKey:@"_temporaryLineIndex"];
	[encoder encodeObject:_retiredWordList forKey:@"_retiredWordList"];
}

-(NSString*)description {
	NSString *description = [NSString stringWithFormat:@"lineIndex:%d temporaryLineIndex:%d wordList.count:%lu", self.lineIndex, self.temporaryLineIndex, (unsigned long)self.retiredWordList.count];
