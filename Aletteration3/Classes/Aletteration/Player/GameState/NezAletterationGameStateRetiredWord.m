...
-(id)initWithCoder:(NSCoder*)decoder {
	if ((self = [super init])) {
		self.lineIndex = [decoder decodeInt32ForKey:@"_lineIndex"];
...
-(NSString*)description {
	return [NSString stringWithFormat:@"NezAletterationGameStateRetiredWord:%d, {%lu, %lu}, %@", self.lineIndex, (unsigned long)self.range.location, (unsigned long)self.range.length, self.stateList];
}
