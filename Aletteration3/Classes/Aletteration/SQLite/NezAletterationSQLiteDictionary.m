...
+(NezAletterationDictionaryInputType)getTypeWithInput:(char*)ins LetterCounts:(NezAletterationLetterBag)letterCounter {
	NSLog(@"   getTypeWithInput:%s", ins);
...
+(long)getPrefixCountWithInput:(char*)ins LetterCounts:(NezAletterationLetterBag)letterCounter {
	long prefixCount = 0;
	unsigned char *letters = letterCounter.count;
	for (int i=0; ins[i]; i++) {
		letters[ins[i]-'a']++;
	}
